...
If the CSR access instruction writes to a register that is wider than XLEN,
such as `instret` on RV32, the update from the side effect is suppressed
for all bits of the wider register, not just the bits written.
